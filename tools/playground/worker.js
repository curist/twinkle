...
// Base URL: two levels up from tools/playground/worker.js → project root
...
async function loadResources() {
  ...
  const [bridgeResp, bootResp] = await Promise.all([
    fetch(BASE_URL + 'tools/bridge.wasm'),
    fetch(BASE_URL + 'target/boot-main.wasm'),
  ]);

  if (!bridgeResp.ok) throw new Error(`Failed to fetch bridge.wasm: ${bridgeResp.status}`);
  if (!bootResp.ok)   throw new Error(`Failed to fetch boot-main.wasm: ${bootResp.status}`);
  ...
  for (const name of PRELUDE_FILES) {
    fetches.push(
      fetch(BASE_URL + `boot/prelude/${name}`)
  ...
  for (const name of PRELUDE_SIG_FILES) {
    fetches.push(
      fetch(BASE_URL + `boot/prelude/signatures/${name}`)
  ...
  for (const name of STDLIB_FILES) {
    fetches.push(
      fetch(BASE_URL + `boot/stdlib/${name}`)
        .then(r => { if (!r.ok) throw new Error(`fetch ${name}: ${r.status}`); return r.arrayBuffer(); })
...
self.onmessage = async (event) => {
  ...
  try {
    ...
    const exitCode = runWasmBytes(null, {
      programArgs: ['boot-main.wasm', 'run', '/input/main.tw'],
      env: { TWINKLE_ROOT: '/' },
      vfs,
      emit,
