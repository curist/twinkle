...
```bash
examples/performance/compiler/racket/run.fish      # side-by-side table: N | twk ms | racket ms | racket/twk
```
...
Each `*.rkt` mirrors the matching `examples/performance/compiler/*.tw` exactly — same `N` values,
