export const warmup = 5, iters = 20, size = 20000, expected = -16908926;
...
export function run(size) {
  ...
  for (let i = 0; i < size; i++) advance(bodies, 0.01);
  // scale energy to an integer checksum (must match the other languages' scale)
  return Math.round(energy(bodies) * 1e8);
}
