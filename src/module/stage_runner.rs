...
mod tests {
    ...
    #[test]
    fn typecheck_key_includes_internal_mode_boundary() {
        let _guard = test_lock().lock().expect("test lock poisoned");
        reset_global_cache();

        let path = PathBuf::from("/virtual/stage_runner_internal_mode/main.tw");
        let source = "println(__host_cwd())\n";

        let permissive_runner = runner_for(&path, source, true);
        let strict_runner = runner_for(&path, source, false);

        let parsed = permissive_runner
            .parse(source)
            .expect("parse should succeed")
            .value;
        let resolved = permissive_runner
            .resolve(
                &parsed.ast,
                TypeEnv::new(),
                ValueEnv::new(),
        ...
        permissive_runner
            ...
            .expect("internal mode typecheck should succeed");

        let strict_err = strict_runner
            .typecheck(
                &parsed.ast,
                resolved,
                default_module_aliases(),
            ...
            .expect_err("strict mode should not reuse internal-mode cache result");
