...
pub fn contract(func_id: FuncId) -> Option<IntrinsicContract> {
    match func_id {
        ...
        id if id == prelude_ids::INT_COMPARE => Some(IntrinsicContract {
            ...
            ret: MonoType::named(crate::types::ty::ORDER_TYPE_ID),
            abi_result: Some(IntrinsicAbiResult::Anyref),
        }),
        id if id == prelude_ids::FLOAT_COMPARE => Some(IntrinsicContract {
            func_id,
            twinkle_name: "Float.compare",
            dispatch: IntrinsicDispatch::Intrinsic,
            type_params: vec![],
            params: vec![MonoType::Float, MonoType::Float],
            ret: MonoType::named(crate::types::ty::ORDER_TYPE_ID),
            abi_result: Some(IntrinsicAbiResult::Anyref),
        }),
        id if id == prelude_ids::STRING_COMPARE => Some(IntrinsicContract {
            func_id,
            twinkle_name: "String.compare",
            dispatch: IntrinsicDispatch::Intrinsic,
            type_params: vec![],
        ...
        id if id == prelude_ids::BYTE_COMPARE => Some(IntrinsicContract {
            func_id,
            twinkle_name: "Byte.compare",
            dispatch: IntrinsicDispatch::Intrinsic,
            type_params: vec![],
            params: vec![MonoType::Byte, MonoType::Byte],
            ret: MonoType::named(crate::types::ty::ORDER_TYPE_ID),
            abi_result: Some(IntrinsicAbiResult::Anyref),
        }),
        ...
        id if id == prelude_ids::VECTOR_GATHER => {
            let t = ty_var("T");
            let vec_t = MonoType::Vector(Box::new(t));
            Some(IntrinsicContract {
                func_id,
                twinkle_name: "Vector.gather",
                dispatch: IntrinsicDispatch::Runtime,
                type_params: vec!["T".to_string()],
                params: vec![vec_t.clone(), MonoType::Vector(Box::new(MonoType::Int))],
                ret: vec_t,
                abi_result: Some(IntrinsicAbiResult::RefArrayNullable),
            })
        }
