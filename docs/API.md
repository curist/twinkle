...
Constants and the pseudorandom generator. The number-theory helpers (`sign`,
`gcd`, `lcm`, `pow`, `isqrt`) now live on `Int` in the prelude, and the
trigonometric / exponential / logarithmic helpers live on `Float`; call them as
inherent methods (`n.gcd(m)`, `x.sin()`) — see the `Int` and `Float` sections
above. This module keeps only the pieces with no natural receiver: the
constants and `random`. Constants are exported with lowercase Twinkle value
names because value identifiers are snake_case.
...
math.random()           // e.g. 0.4271…

// The former math.* functions are now inherent methods:
12.gcd(18)              // 6
2.pow(10)               // 1024
99.isqrt()              // 9 (floored integer root)
(math.pi / 2.0).sin()   // 1.0
...
site (a `defer` runs when its enclosing `{ }` block exits). At top level, the module
initialization sequence acts as the enclosing block, so deferred cleanup runs when it
completes. A freed region may be reused by the next `buffer.new` call (address-ordered
free-list coalescing), so use-after-free silently reads or corrupts the next allocation.
