...
- **S4 — thaw-from-`PVec` / param-sourced owned vectors.** S4 slice 1 (caller-born, single-hop; owned-specialized mutable ABI across the call) **landed** — `sieve` reaches the flat-`MutVec` floor, HP-5 sibling clones handle mixed callers, and the completed execution plan is archived at `archive/interprocedural-mutvec.md`. The detector still requires the vector to be **locally born** (collect/make) so ownership is trivial. The remaining S4 work: (a) **param-sourced thaw** — claiming an owned vector arriving as a `PVec` parameter (the `nbody` `advance` shape), which needs a cross-function uniqueness proof plus an entry thaw→mutate→refreeze; (b) a **non-Int-family fixture** (Test B: a `Float`/`Byte` user wrapper) to prove family-generality end-to-end rather than by construction; (c) a **compiled-and-run checksum guard** in the boot suite (the S4 suite is WAT/ANF-text-only today). All live on the **sound-uniqueness track** (`docs/plans/sound-uniqueness/`, storage S4); revisit as an S4 customer, not here.
