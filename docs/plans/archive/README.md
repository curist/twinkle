...
| [boot-typed-builtin-type-refs.md](boot-typed-builtin-type-refs.md) | Extend the by-name builtin-ref pattern to the remaining builtin *types* (`Order`/`Iterator`/`Range`) via trapping `order_type`/`iterator_type`/`range_type` accessors in `builtin_refs.tw`; thread env into `contract_return_type`/`synth_range_op` and the `lower_ord_cmp`/IntoIterator lowering sites. `IterItem`/`UnfoldStep`/`Task` audited (already name-based). Self-host fixed point, ids unchanged |
...
| [shared-wrapper-owned-specialization.md](shared-wrapper-owned-specialization.md) | Phase 8G shared-wrapper owned specialization: publish owned variants for wrapper functions whose collection param threads through optimizer-declared COW update calls and returns, so owned callers route to in-place clones while aliased callers stay generic |
| [2026-07-22-builder-region-linearly-folded-fact.md](2026-07-22-builder-region-linearly-folded-fact.md) | Codegen 8C Plan 1: the **structural** `linearly_folded` builder-region fact (pure-ANF detector + `twk ir --census --sites` render, no ownership dependency — condition 2 dropped mid-execution as unnecessary/unsatisfiable-for-strings) plus range/index-loop coverage; Plan-2 rewrite deferred, follow-ups in [2026-07-22-8c-plan1-review-followups.md](2026-07-22-8c-plan1-review-followups.md) |
| [2026-07-22-8c-plan1-review-followups.md](2026-07-22-8c-plan1-review-followups.md) | Codegen 8C Plan 1 deep-review disposition: F1/R1/F2 (done in Plan 1), FU-1 (fold-result deadness gate) + FU-2 (re-fold surfacing) **done in Plan 2**; FU-3 (ANF-walker unification) deferred/unscheduled — relocated to the builder-region design doc's roadmap |
| [2026-07-22-8c-plan2-builder-region-rewrite.md](2026-07-22-8c-plan2-builder-region-rewrite.md) | Codegen 8C Plan 2 execution plan: producer + ANF-to-ANF builder-region rewrite (string + vector empty-seed) wired into `link_program` (ANF′), FU-1 gate + FU-2 surfacing + `repr_assign` string-seed fix + consumed-column inspection; string/vector accumulator loops lower to builder calls, self-host fixed point holds |
| [mutdict-bottom-up-publication-adapter.md](mutdict-bottom-up-publication-adapter.md) | Settled dense-to-persistent Dict publication design; archived after `freeze_dense`, bulk order construction, compact routing, and adversarial builder coverage landed. Retains the historical forward S5 lifecycle, arena-consumption, profiling, and cleanup contract. |
| [2026-08-28-mutdict-publication-adapter-extraction.md](2026-08-28-mutdict-publication-adapter-extraction.md) | Completed implementation slice for the compiler-private publication adapter, engineered-hash test seam, builder-shape fixtures, and mutation-proven compaction publication regression. |
| [dict-bottom-up-hamt-builder.md](dict-bottom-up-hamt-builder.md) | Successful bottom-up HAMT construction spike: counting-partition by 5-bit hash fragments, exact-size node/slot allocation, bulk order build — materially cheaper dense publication, shifting the flat→persistent crossover to k/n ≈ 0.12–0.15 at 1M. Production adapter landed (see the publication-adapter rows above); residual spike-surface cleanup stays gated on S5 being reopened. |
| [2026-08-29-mutdict-evidence-closure.md](2026-08-29-mutdict-evidence-closure.md) | Completed execution plan that closed the S5 MutDict representation gate: full same-session Candidate M vs Candidate H vs persistent arena measurements (timed construction, k/n=1/8·1·4, churned-physical clone, HAMT/order split) plus the boot-compiler workload census. Verdict folded into `mutdict-dense-freeze-input.md` and `sound-uniqueness/storage/README.md`: **STOP WITHOUT SELECTION** — the boot compiler is build-once/low-overwrite, so no update-dense customer justifies building the runtime. |

## Archived Cross-Cutting Plans

| Plan | Description |
|------|-------------|
| [awfy-benchmark-suite.md](awfy-benchmark-suite.md) | Design for the AWFY-style cross-language benchmark suite (9 micros + Json) comparing Twinkle's Wasm-GC output against Node/V8 and Go for compiler perf gap-finding; shipped at `examples/performance/awfy/` |
...
| [awfy-codegen-gaps.md](awfy-codegen-gaps.md) | Runtime codegen gaps surfaced by AWFY vs Node/Go. Landed: dead-Void elim + native `Float.sqrt`/etc. replacing JS FFI Math calls (nbody 6.6×). Rejected as V8-already-handled: peephole/struct.get caching/record-alloc elimination (V8 scalar-replaces non-escaping GC structs). C5 spun out (below); `rt_arr__get`/typed-storage deferred to the representation track |
| [awfy-c5-inplace-vector.md](awfy-c5-inplace-vector.md) | In-place vector `set_at` (the sieve/bounce lever). 1a fixed a pre-existing soundness bug (in-place fired on vectors aliased via slice/concat/record/variant/closure — `refreshed` overrode taint); 1b made method-form `set_at` in-place-eligible via thin-wrapper detection + non-escaping element-read allowance. **sieve ~7×, bounce ~9×.** Phase 2 measured + not built: record-alloc elimination is a non-lever; inliner+SROA ~15% (deferred); remainder ~2.3× is representation |
| [dataframe/](dataframe/README.md) | App-scale columnar query-engine stress test (design, impl plan, friction log). **Retired 2026-09-03 — findings delivered:** durable ergonomics friction (no cross-module fluent chains, circular-import hard error, `Result` breaks chaining) captured in its friction log; the `order_by` sort-bound perf finding drove `sort-bench` and the still-active [vector/sort track](../performance/vector/README.md). Working engine/benchmarks remain live at `examples/performance/dataframe/`; only the plan docs are archived |
