...
`target/twk run examples/performance/compiler/<name>.tw`. Use `hyperfine` only for whole-process sanity.

Benchmarks (each parameterized by N, run at N = 1k, 2k, 4k, 8k, 16k, 32k):

```tw
// examples/performance/compiler/concat_prepend.tw — the target case. Expect ~4× per doubling (quadratic).
acc: Vector<Int> = []
for i in range(n) { acc = [i].concat(acc) }       // right-operand accumulator

// examples/performance/compiler/concat_append.tw — control. Must stay linear on BOTH old and new.
acc: Vector<Int> = []
for i in range(n) { acc = acc.concat([i]) }

// examples/performance/compiler/slice_droplast.tw — the LIFO case. Expect ~4× per doubling (quadratic).
acc: Vector<Int> = range(n).to_vector()
for ... { _ = acc[acc.len() - 1]; acc = acc.slice(0, acc.len() - 1) }   // drop-last

// examples/performance/compiler/slice_dropfirst.tw — acc = acc.slice(1, acc.len()) in a loop (left-drop).
// examples/performance/compiler/droplast_baseline.tw — same drop-last workload via the proposed
//   `drop_last` op, to show the O(log n) target it would hit.

// examples/performance/compiler/concat_balanced.tw — pairwise/tree concat of many small vectors.
// examples/performance/compiler/get_regular.tw — N random get() on an append-built (regular) vector.
// examples/performance/compiler/get_relaxed.tw — N random get() on a concat-built vector (post-RRB:
//   exercises relaxed-node navigation; pre-RRB: baseline get cost).
// examples/performance/compiler/set_regular.tw, examples/performance/compiler/set_relaxed.tw — same for set().
```
...
— the complexity is not paid for. Keep the benchmark suite in `examples/performance/compiler/` as a
permanent regression guard regardless of outcome.

#### Gate B baselines established (2026-05-30)

The suite is built and run — see `examples/performance/compiler/` (`README.md` has the full table,
...
checked. The `examples/performance/compiler/` suite from Gate B becomes the before/after evidence and a
