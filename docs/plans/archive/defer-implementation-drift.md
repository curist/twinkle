...
**Status: CLOSED** — All exit criteria met as of 2026-04-20.

Implementation summary:
- `in_sub_expr: bool` → `branch_start: Option<usize/Int>` in both Rust and boot
  defer-elim passes; terminal Atom inside a branch fires only branch-local defers.
- `src/interp/eval.rs`: `if`/`match` branches push/pop a defer scope.
- `src/opt/pipeline.rs` / `boot/compiler/opt/pipeline.tw`: `eliminate_defers`
  moved before peephole passes to prevent `branch_simplify` hoisting `ADefer`
  nodes out of branches.
- Tests: `defer_if`, `defer_break`, `defer_continue` added to both interpreter
  and wasm runners; boot ANF-level tests for block-scope, break-only-loop-defers,
  continue-only-loop-defers added to `opt_defer_elim_suite.tw`.
- Docs: `docs/design/defer.md` implementation notes updated; `docs/open-questions.md`
  item 6 updated to reflect `defer` is implemented.
