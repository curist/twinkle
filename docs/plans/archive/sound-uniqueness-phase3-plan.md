...
- **G7 — Intraprocedural fixpoint updates must converge without blanket precision loss.** Exact replacement is allowed while a block/local fact is converging. Track each block's previous stored exit; when a local fact flips back to its previous value, lock only that local to conservative widening on later visits: ownership via `join_own`, binding-validity via meet/`and`, provenance via sorted union. Also keep a bounded per-block changed-visit counter; after the cap, force-lock any still-changing locals in that block so period-3+ cycles fall back to sound widening too. Direct replacement can oscillate on loop CFGs (`Unknown ↔ Shared`) even when the interprocedural SCC driver is correct; blanket widening all already-processed facts is sound but unnecessarily loses loop-carried `Unique` precision.
...
Add the prov twins (`prov_map_get`, `same_prov_map` — reuse `same_live` for the inner vector compare, `join_entry_prov`, `prov_of_in`, `seed_param_prov`) exactly as in the design; extend `FixResult` with `exit_prov` and `run_fixpoint` to (a) take the function `params`, (b) seed block 0's entry prov via `seed_param_prov`, (c) join entry prov via `join_entry_prov`, (d) detect and target-widen oscillating block/local facts before change-detection (G7), (e) compare prov in change-detection, (f) store the exact-or-target-widened `exit_prov[blk]`.
...
Add targeted oscillation helpers before wiring the loop body. These helpers default to exact replacement (`out := next`), and only widen locals that are already locked, that just flipped back to their previous value, or that are still changing after the per-block changed-visit cap:

```tw
// Sketch; implementation has ownership/valid/prov variants.
// `has_prev` is false until a real prior change has been recorded, so the empty
// initial previous map is not mistaken for an oscillation.
oscillates := has_prev and old_fact != next_fact and next_fact == prev_fact
changed := old_fact != next_fact
if locked(local) or oscillates or (force_lock_changed and changed) {
  lock(local)
  out[local] = conservative_merge(old_fact, next_fact)
} else {
  out[local] = next_fact
}
```

`run_fixpoint(blocks, params, table, b, sem)` per-block step (mirrors the own/valid handling, with G7 targeted widening):
...
```tw
      ...
      if already {
        ...
        force_lock_changed := change_count_get(changed_visits, blk.id.id) >= 8
        own_merge := merge_own_exit_targeted(old_own, st.own, own_map_get(prev_exits, blk.id.id), locked_get(locked_own, blk.id.id), has_prev, force_lock_changed)
        // valid/prov use the same targeted shape with meet/union as their conservative merge.
...
First inspect the SCC API: `grep -n "pub fn\|pub type" boot/compiler/graph_scc.tw`. **Note it is String-keyed** — `strongly_connected(nodes: Vector<String>, edges: Dict<String, Vector<String>>) Vector<Vector<String>>` — so `order_sccs` must marshal `FuncId.id ↔ String` in both directions. Use a plain decimal encoding of `FuncId.id` (`fid.id.to_string()`, no zero-padding) as the node label, enumerate `nodes` in **ascending `FuncId.id` order**, and build `edges[fid_str] = callee_ids(f) mapped through the same encoding` so the returned SCC order — and therefore the summary-fixpoint order — is byte-stable across builds. Decode each returned component's labels back to ids (parse the decimal string) to drive the fixpoint. Then **create `boot/compiler/summary.tw`** importing `compiler.ownership` (types + `summarize_function`), `compiler.cfg` (`CfgView`/`CfgFunction`), `compiler.anf`, `compiler.core_ir.{FuncId}`, `compiler.builtins`, `compiler.opt.semantics`, and `compiler.graph_scc`. Implement `pub fn compute(view, b, sem) SummaryTable` per the design: build the user-id set, seed each function's summary conservatively (`params Retained`, `ret Shared`), order by call-graph SCCs (callee-first), and for each SCC iterate `ownership.summarize_function` until stable or a `members*4+1` cap, replacing changed summaries. Provide `conservative_summary`, `same_summary` (compare escape/cap tags + return tag + alias-set), `callee_ids` (scan `ACall(AGlobalFunc(fid))` for user ids), `func_by_id`, and `order_sccs` (adjacency `fid → callee_ids`, `FuncId.id ↔ String` marshalling as above, Tarjan via `graph_scc.tw`, callee-first order). Full code as in the design's driver sketch. (`empty_summary_table` already lives in `ownership.tw` — do not redefine it here; `summary.tw` never needs to be imported by `ownership.tw`, keeping the graph acyclic per G6.)
...
**Optimization reach (do not be alarmed by the demotes).** The narrow, real win here is that read-only (`Borrowed`) args stay `Unique` and `OwnedFresh` results are `Unique`. The census-dominant consume-produce helpers — `set_at`, `add_type`, `visit` (the 171 record + 267 dict in-place sites) — *return their threaded param*, so they classify `p0=retain, ret=alias(p0)` and are **fully demoted at the caller by design** (same verdict as today's blanket publish boundary — no regression, no win). Recovering their unique handoff needs ownership specialization (re-analyzing the callee under a `Unique`-entry assumption), which is **Phase 6**; Phase 3 emits no codegen (Task 8 gate: 0 in-place), so nothing is lost. See the design's "Optimization reach" section and worked-examples Cases A/B/V.
...
- **Task 1 threads `table` + `prov` through many signatures** (`run_fixpoint`/`forward_block`/`ownership_stage`/`analyze_function`/`transfer_op`/`transfer_call`). Grep and update all call sites in the same task to keep it compiling. Also reword the stale "FROZEN … guardrail G1" comment on `analyze_function`.
...
- **Inner fixpoint convergence:** the original plan review checked the interprocedural SCC fixpoint, but missed that the intraprocedural ownership fixpoint could oscillate because it replaced already-processed block exits with the latest transfer result. Reviewers must check the update operator itself: exact replacement is fine while facts converge, repeated flips must be detected and target-widened per local, and a changed-visit cap must force-lock still-changing locals so convergence does not depend on cycles being period-2. Add a real loop-heavy regression (`resolver.remap_type_def`) whenever this driver changes.
