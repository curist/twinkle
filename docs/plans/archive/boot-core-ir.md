...
### Resolved issues (all fixed 2026-03-21)

All gaps and discrepancies from the initial review have been resolved:

**G1+G2. method_calls map + method call lowering** — Added `method_calls:
Dict<Int, String>` to `InferCtx`, `CheckResult`, and `LowerCtx`.
`try_synth_method_call` records resolved method names. `lower_call` detects
method calls and emits `Call(GlobalFunc(fid), [receiver, ...args])`.

**G3. Complex lvalue assignment** — Added `lower_lvalue_assign` + `LvalueOut`
type. Handles Field (RecordUpdate), Index (VECTOR_SET_UNSAFE), Dict (DICT_SET).

**G4. For-loop type dispatch** — `lower_for` dispatches on `iter_expr.ty`:
String uses STRING_LEN (FuncId 8), Dict calls DICT_KEYS (FuncId 14).
Note: Range and Iterator forms deferred (need builtin types in test env).

**G5. Collect type dispatch** — `lower_collect` dispatches on iterator type:
String uses STRING_LEN, Dict calls DICT_KEYS + VECTOR_LEN.

**G6. Type alias resolution** — `resolve_type_id` follows `Alias` definitions
to canonical `Named(tid, _)` TypeId.

**D1. Terminal-aware expr-stmts** — Added `is_terminal` helper. Expr-statement
paths emit terminals directly without `Let` wrapping.

**D2. QualifiedVariant qualifier** — `lower_pattern` resolves TypeId from
qualifier path via `resolve_type_id`, falling back to scrutinee type.

**D3. Monomorphization TypeId guard** — Added `tid_p.id != tid_c.id` guard.

**D4. Index variable binding** — Both `lower_for` and `lower_collect` bind
user index local to loop counter.

**D5. Field punning type** — Uses `find_field_type(env, tid, name)`.

**D6. Byte in string interpolation** — Routes to FuncId 1024.

**D7. LitByte removed** — Removed unreachable variant from CoreExprKind.
