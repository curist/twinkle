...
`examples/performance/dataframe/bench/order_by_breakdown.tw` is a set of component probes, not an additive profiler for a single `order_by` invocation. The useful mental model for current `order_by` is:
...
`examples/performance/sort-bench/value_sort_micro.tw`; Twinkle COLD single-run via `target/twk run`;
Clojure persistent-vector via `examples/performance/sort-bench/value_sort_clojure.clj`, WARMED):
...
in-process (`examples/performance/sort-bench/sort_repeat_probe.tw`, 2026-06-10) shows it settle at
...
target/twk run examples/performance/dataframe/bench/order_by_micro.tw
...
clojure examples/performance/dataframe/bench/order_by_clojure.clj
go run examples/performance/dataframe/bench/order_by_go.go
rustc -O examples/performance/dataframe/bench/order_by_rust.rs -o /tmp/order_by_rust && /tmp/order_by_rust
```
