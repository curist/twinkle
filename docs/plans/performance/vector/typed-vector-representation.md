...
- `PhysPlan` is the **prepare-time decision record** (an `ElemRepr` per func-id-keyed site: slots, returns, captures — each single-family because functions monomorphize). Emit and verify read `SlotInfo.repr` / `wasm_type` (consistent by the invariant), not the plan object; `mutvec_repr` is the one downstream reader of the plan itself, and only as a non-decisional guard (see below) — it never derives a repr from it.
