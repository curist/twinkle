...
- `PhysPlan` is the **prepare-time decision record** (an `ElemRepr` per func-id-keyed site: returns, captures — each single-family because functions monomorphize). Emit, verify, and `mutvec_repr` read `SlotInfo.repr` / `wasm_type` (consistent by the invariant), not the plan object.
