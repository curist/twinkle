...
target/twk run examples/performance/dataframe/bench/sort_by_costs.tw
...
`examples/performance/sort-bench/merge_attribution_probe.tw` ablates the real recursive merge sort (validated: its full variant reproduces `sort_by`, ~720 vs ~731 ms). At N = 1M:
...
structure. `examples/performance/sort-bench/sort_repeat_probe.tw` runs each phase three times
...
`examples/performance/sort-bench/enum_alloc_probe.tw` isolates producing+consuming a comparison result as an enum vs a bare `Int`, at the ~17M-comparison scale, with identical branch structure. Stable results:
...
- Keep/create: `examples/performance/dataframe/bench/sort_by_costs.tw`
- Keep: `examples/performance/sort-bench/sort_by_component_probe.tw`, `examples/performance/sort-bench/enum_alloc_probe.tw`
...
- [ ] Add a Clojure side-effect comparator reference script under `examples/performance/dataframe/bench/` if we want the comparison to be repeatable.

### T0.2 Attribute the ~720 ms generic-mechanics half — done

`examples/performance/sort-bench/merge_attribution_probe.tw` ablates the merge (R = reads+compare+recursion, S = +singletons, B = +append/alloc; B validated against real `sort_by`). Result at N = 1M:
...
```bash
target/twk run examples/performance/sort-bench/value_sort_micro.tw
clojure examples/performance/sort-bench/value_sort_clojure.clj
```
