...
| [typed-vector-elem-families.md](typed-vector-elem-families.md) | Generalize the Int-hardcoded routing into an element-family layer, then land `PVecBool` (the `nulls` mask); core read path landed, dataframe null-mask win deferred | partial/landed core |
| [typed-vector-make-and-bool-parity-design.md](typed-vector-make-and-bool-parity-design.md) | Typed `Vector.make` + Bool field/payload/gather parity | **Track A landed** (`eebcc061`); Bool parity holds, but the real dataframe null mask is blocked by a family-neutral **tail-return-typing gap** (a tail-returned `Vector.make` isn't given a typed-return ABI) — fix in progress (see doc's Outcome) |
| [top-level-typed-vector-routing-scope.md](top-level-typed-vector-routing-scope.md) | Scope for routing typed vectors at **top level**: root cause (top-level bindings are boxed module globals, so a typed producer escapes at `AGlobalSet`) + options (typed-global ABI vs init-local rewrite vs document) | scoping, low priority |
...
- `examples/performance/sort-bench/typed_{int,bool}_make_read_probe.tw`, `typed_bool_make_boxed_probe.tw` — typed `Vector.make` (Track A): a make-produced vector read by index keeps `PVecI64`/`PVecBool` (`rt_arr__make_i64`/`make_bool` + `get_i64`/`get_bool`); the boxed probe shows a typed make value passed to a normal `Vector<Bool>` parameter boxes via `rt_arr__box_bool`.
- `examples/performance/sort-bench/typed_bool_field_payload_probe.tw` — side-by-side Int/Bool parity across record field, variant payload, structural `==`, and gather-on-typed-receiver; every Bool family call count equals its Int analogue. (Must be function-scoped: top-level producers are boxed module globals — see [top-level-typed-vector-routing-scope.md](top-level-typed-vector-routing-scope.md).)
- `examples/performance/sort-bench/typed_bool_null_mask_probe.tw` — dataframe-shaped null mask: `Vector.make` → typed record field → indexed read → gather, routing `make_bool`/`get_bool`/`gather_bool` exactly as the Int baseline.
