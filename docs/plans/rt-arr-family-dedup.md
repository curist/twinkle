...
- **The acceptance gate for every fold is byte-identical emitted WAT.** For each merged function, `target/twk wat boot/main.tw --func <exact_name>` must be identical before and after. A fold that changes emitted bytes is wrong even if tests pass — capture the baseline first, diff after. This is what makes the refactor provably a no-op.
