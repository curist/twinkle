...
| String builder region | Persistent `String.concat` accumulator shape | `string$builder_from [str?]→[sb]`, `builder_extend [sb?, str?]→[]`, `builder_freeze [sb?]→[str]` | Region: `from(base)` → `extend*` → `freeze` | Same region shape as vector builders but no string in-place mutation; only builder lowering is in scope. |
