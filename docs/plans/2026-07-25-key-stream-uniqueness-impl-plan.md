...
- [ ] **Step 1:** Write each fixture as a compilable `.tw` (each helper called from a `main` so it isn't DCE'd). Design §7 gives the exact shape of each. `target/twk fmt` + `target/twk run <fixture>` each to confirm it compiles and runs.
- [ ] **Step 2:** Write the classification test (mirror the existing `phase8g` test in `cfg_summary_suite.tw`: `pipeline.compile_entry_path` → `cfg.build_view` → `ownership.classify_dedupe_helpers`). Assert positives certified (via certificate presence — Task 2 introduces the type; until then assert `table_is_dedupe_helper`), negatives absent.
- [ ] **Step 3:** `target/twk run boot/tests/main.tw`. Expected: the **new negatives that the current unsound checker wrongly certifies** (e.g. `flag_reset_in_loop`, `bad` via the old flag path, `bad2`) FAIL (red); positives pass. This red set is the target.
...
- [ ] **Step 3:** Update the Task-1 test to assert on the certificate (kind). `fmt` + `lint` + `target/twk run boot/tests/main.tw`. Expected: **all negatives green** (default-deny rejects everything), **all positives red** (nothing certified yet). This is the clean TDD baseline.
