...
| [2026-07-29-field-tier-recursive-self-routing.md](2026-07-29-field-tier-recursive-self-routing.md) | 8H follow-up: route a field-tier variant clone's in-SCC recursive call back to itself so owned recursive record-field collection updates stay in-place at every depth (`rec=1`). Seeds `field_seed_for_variant` into the recursive-route ownership analysis. |
| [2026-07-29-loop-threaded-field-ownership.md](2026-07-29-loop-threaded-field-ownership.md) | 8H follow-up: prove loop-carried/threaded reference-typed field backing uniquely owned so a record-backed collection update on that field lowers in place across iterations (fixes loop-carried `Set.insert`/record-field dict). Investigation-led fixpoint precision work. |
