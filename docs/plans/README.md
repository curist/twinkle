...
| [interprocedural-mutvec.md](interprocedural-mutvec.md) | **Plan draft.** Execution plan for the first **storage S4** slice (owned-specialized mutable ABI across calls). Follows the landed shared-wrapper lever: the owned `set_at` clone now emits persistent `vector$set_in_place` (sieve 35→5ms) but can't reach flat `MutVec` (0.86ms) because MutVec regions are locally-born + intra-function (`mutvec_region.tw:148`). Lets a flat `MutVec<fam>` handle flow into/out of an owned-specialized callee so the chain stays low (sieve is a no-escape scratch chain → zero freezes) — targets sieve→`sieve_mut` parity and unblocks nbody's param-sourced `advance`. Needs a post-8G interprocedural phase with a continuation verifier, route partitioning, and a new MutVec-ABI decision input; **not** summary.tw work (analysis is done). Wrapper inlining is a separate doc. |
