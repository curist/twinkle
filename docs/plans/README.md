...
| Linear-memory Buffer | Linear-memory buffers foundation-first (byte-addressed `Buffer` + typed views). M1 (infra) + M3 (byte-codec proxy, ~30×) validated; M2 — the opt-in, manually-managed, first-class user-facing `Buffer` in `@std.buffer` — is active | M2 (plan) | spec [buffer-linear-memory.md](buffer-linear-memory.md), plan [buffer-linear-memory-m2-plan.md](buffer-linear-memory-m2-plan.md) |
