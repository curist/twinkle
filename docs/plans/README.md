...
| Rebinding-ceremony fixers | Skeleton: three `twk lint`/`fix` rules from manual "tidy up" rewrites — named-ctor to anonymous `.{}` (A), numbered-accumulator rebind (B), full-record reconstruction to field rebind (C). A landed as `redundant-record-prefix` (three anchors: annotated `let`, declared return, record-field value; call-argument anchoring deferred from v1), applied across boot (2026-08-06, re-applied 2026-08-07). C's return-position variant landed via `record-copy-helper` (2026-08-06). B landed as `redundant-rebinding` (report-only detector + Shape-1 auto-fix under `--fix-redundant-rebinding`, 2026-09-12; pure aliases fire regardless of naming, computed advances keep the sibling heuristic; spec archived at [archive/lint-numbered-rebinding.md](archive/lint-numbered-rebinding.md)); C's let-binding/nested/block-expr variants remain | Planned (A done, B done, C partial) | [lint-rebinding-fixers.md](lint-rebinding-fixers.md) |
...
| [task-channel-api-maturation.md](task-channel-api-maturation.md) | **Planned — concurrency API maturation.** Grow the `Task`/`Channel` MVP: repair substitution handling for inferred function returns consumed through generic closures (WS-B), add recoverable task failure `try_await` (WS-C), decide `send`-on-closed ergonomics (WS-D), and design the two missing capabilities — `select` over channels (WS-E) and structured concurrency/cancellation (WS-F). Channel API documentation (WS-A) is done; WS-B is a core inference fix, while WS-E/F are design-first. Independent workstreams; stage0 only bites if boot compiler/command source adopts a new primitive (compile-time, not runtime). |
