...
| Disk-backed debug info | Follow-on: drop inline source text from `twinkle.debug`. **Milestone 1 (complete)** — keep absolute paths + line/col + PC maps, and read snippets from disk at render time (scoped to `twk run`, 3-arg renderer unchanged, no host-boundary change). Self-hosted and verified: `boot.wasm` dropped from ~9.0 MB to ~6.7 MB; boot + JS suites green; `twk run` snippets confirmed end-to-end. `twk build` artifacts and other runs degrade to `file:line:col` + backtrace. **Milestone 2 (deferred)** — project-relative paths + `source_root` safe-join + `find_project_root` fix for portable/shipped artifacts. No hashes/strip mode; staleness documented not detected. | Milestone 1 done, Milestone 2 planned | [disk-backed-debug-info.md](disk-backed-debug-info.md) |
