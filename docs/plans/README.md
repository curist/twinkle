...
| [interprocedural-mutvec.md](interprocedural-mutvec.md) | **Slice 1 landed** (caller-born, single-hop; `sieve` 4.86→0.62ms = `sieve_direct` flat floor, self-host green). Execution plan for the **storage S4** track (owned-specialized mutable ABI across calls). A flat `MutVec<fam>` handle now flows into/out of an owned-specialized single-update clone: the accepted caller region + upgraded clone body move to `mutvec_*` ops (post-8G phase `s4_phase`, continuation verifier `s4_verify`, all-sites route guard, MutVec-ABI decision consumed in `mutvec_repr`). No-escape scratch → zero freezes; escape → one boundary freeze. HP-5 sibling clones for mixed callers landed (a `$mv` MutVec-ABI sibling; non-S4 sites keep the persistent clone); producer discovery narrowed to lowerable boxed-`collect`/`make` seeds (array-literal/typed-freeze fall back to persistent). **Next: param-sourced `nbody` `advance`** (thaw a `PVec` parameter at entry) + non-Int family fixture (Test B) + compiled-and-run checksum guard. |
