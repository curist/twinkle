...
| Task concurrency | Add stackful cooperative `Task<T>` concurrency without new syntax; current backend uses JSPI fibers | Planned | [design](task-concurrency-jspi-fiber.md), [implementation](task-concurrency-jspi-fiber-implementation.md) |
...
| Linter | `twk lint`: clippy-style review command — reports report-only lints and auto-fixable rewrites (with `--fix`/`--fix-<rule>`) across the import chain. Opt-in, no config. Built: command, rewrites R1 inherent-method-call (design in [archive/inherent-method-hint.md](archive/inherent-method-hint.md)) + R2 unused-imports, lints L2 ignored-Result/Option + L3 record-copy-helper + L4 unreachable-code + L5 direct-rebinding. L1 unused-pure-result dropped for now (needs whole-program effect analysis; see linter.md §6) | Mostly done | [linter.md](linter.md) |
