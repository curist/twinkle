...
|------|-------------|--------|---------|
| In-buffer crypto | `Buffer.get_u32`/`set_u32` + `crypto.{md5,sha1,sha256}_buf(Buffer)` hashing data already in linear memory (word-loaded message + in-place buffer schedule), to win the 4k crypto-bench cases. `_bytes` paths untouched | Design (approved) | spec [in-buffer-crypto.md](in-buffer-crypto.md) |
