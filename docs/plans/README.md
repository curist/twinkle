...
| Rebinding-ceremony fixers | Skeleton: three `twk lint`/`fix` rules from manual "tidy up" rewrites — named-ctor to anonymous `.{}` (A), numbered-accumulator rebind (B), full-record reconstruction to field rebind (C). A landed in full as `redundant-record-prefix`, applied across boot (2026-08-06). C's return-position variant landed via `record-copy-helper` (2026-08-06); B and C's let-binding/nested/block-expr variants remain | Planned (skeleton; A done, C partial) | [lint-rebinding-fixers.md](lint-rebinding-fixers.md) |
