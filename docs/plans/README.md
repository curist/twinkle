...
| Disk-backed debug info | Follow-on: drop inline source text from `twinkle.debug`; store project-relative paths + line/col + PC maps only, and recover snippets from disk at render time — scoped to `twk run`. Shrinks the boot compiler's ~2× debug bloat; `twk build` artifacts and other runs degrade to `file:line:col` + backtrace. No hashes/strip mode yet. | Planned | [disk-backed-debug-info.md](disk-backed-debug-info.md) |
