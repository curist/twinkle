...
| [fixpoint-map-inplace.md](fixpoint-map-inplace.md) | Diagnosis doc, **re-scoped 2026-07-26 to the general analysis goal**: make the ownership analysis prove owned collections unique so the compiler emits in-place writes reusably (not hand-applied per function; the compiler's fixpoint is one beneficiary). **Primary lever = the aggregate-field owned-variant extension** (recognize a fresh-aggregate return whose fields are owned carriers, e.g. `merge_targeted__`'s `ret_paths=.f0=from(p1) .f1=from(p3)`, and fire at call sites passing those inputs uniquely); sized bounded — the variant identity, `ret_paths`, and caller side already support multi-carrier; the gap is `candidate_variants`/`optimistic_hypothesis`/`variant_valid` + the `run_scc_variants` single-param driver. Lever D landed. The `run_fixpoint` cold/warm split is demoted to a non-universal source workaround. |
| [aggregate-field-owned-variants.md](aggregate-field-owned-variants.md) | Implementation plan for the general **aggregate-field owned-variant** lever: teach the owned-variant machinery to recognize a function that returns a fresh aggregate whose fields are owned carriers (`ret=OwnedFresh ret_paths=.f0=from(p1) .f1=from(p3)`), so its internal collection updates emit in-place at call sites that pass those inputs uniquely. Proven on the minimal 2-carrier `merge_targeted__` fixture; helps builder/transform functions returning a record/tuple of collections, and (via a returned-carrier helper) `run_fixpoint`. Concentrated in `candidate_variants`/`optimistic_hypothesis`/`variant_valid` + the `run_scc_variants` single-param driver (`member_pidx`); central risk is N-carrier validation soundness. |
