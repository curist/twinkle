...
| [2026-08-28-mutdict-publication-adapter-extraction.md](2026-08-28-mutdict-publication-adapter-extraction.md) | Executable slice of the [MutDict bottom-up publication adapter design](mutdict-bottom-up-publication-adapter.md) (storage-track **S5**). Extract `freeze_dense` + `build_order_bulk` from `Dict.compact()`, route compact through them, and add an engineered-hash test seam covering the previously-untested deep-prefix and full-hash-collision builder paths. Boot-only; the forward MutDict lifecycle/poison ABI, arena consumption, and spike-surface cleanup stay blocked on the unbuilt S5 MutDict runtime. |
