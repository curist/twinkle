...
| Boot typed builtin refs | Resolve well-known builtin type/variant ids into typed compiler references instead of relying on implicit numeric layout assumptions | Planned | [boot-typed-builtin-refs.md](boot-typed-builtin-refs.md) |
...
| [collection-ergonomics.md](collection-ergonomics.md) | Add common `Vector` and `View` helpers (`take`, `drop`, `zip_with`, `chunks`, `windows`, and owned-vector conveniences) while keeping `View` as the zero-copy read-only sequence/window abstraction |
