...
| [awfy-benchmark-suite.md](awfy-benchmark-suite.md) | AWFY-style cross-language benchmark suite (9 micros + Json) comparing Twinkle's Wasm-GC output against Node/V8 and Go, for compiler perf gap-finding. Per-language dirs, shared `(warmup, iters, size)` table, `run.sh` failing on cross-language checksum disagreement |
