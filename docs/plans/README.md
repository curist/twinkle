...
| [stack-safety-transform.md](stack-safety-transform.md) | **Primary stack-safety direction.** An ANF→ANF pass that auto-rewrites statically-unbounded (SCC, non-tail) recursion into a driver loop over a `@std.buffer` shadow stack, so deep recursion stops overflowing V8's Wasm stack at runtime *and* compile time (the boot compiler is itself a user program). Spiked to ~1.05× tax with a Buffer stack + mode-as-control-flow; hybrid depth-threshold keeps shallow recursion native. Prerequisite typed-repr grow-rebind fix LANDED. Phased: differential harness → tier-1 self-recursion → tier-2 SCC → hybrid threshold. |
