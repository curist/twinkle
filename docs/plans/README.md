...
| [fast-read-text.md](fast-read-text.md) | Add a privileged `twinkle_runtime.read_file_string` host op that reads a file in one call and returns a validated GC `String` directly, skipping the `Vector<Byte>` PVec rebuild (~65ms of the read cost) and the Twinkle-side `from_utf8` walk. `fs.read_text` becomes a thin wrapper (~4× faster); `read_bytes` stays for binary. Touches both compilers (extern-safety allowlist + builtin ABI) plus `runtime.mjs` + `fs.tw`. Complementary to `mutvec-later-slices` Phase 4 (`PVecByte`, the binary path). |
