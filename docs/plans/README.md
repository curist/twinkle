...
| [rt-arr-family-dedup.md](rt-arr-family-dedup.md) | Byte-identical refactor folding the per-element-type copy-paste in `runtime/arr.tw` (PVec trie ops) into the `PVecFamily` builder, enriching `PVecFamily` with `suffix`/`leaf_store`/`elem_box`. Its payoff — cheaply adding a new element family (e.g. `PVecF64`) — is the enabler for the MutVec later slices. |
| [mutvec-later-slices.md](mutvec-later-slices.md) | Successor to the landed MutVec slice-1 (i64, unconditional): generalize the seven `mutvec_*` ops over `PVecFamily` for `Bool`/`Float`/boxed element families (depends on rt-arr-family-dedup; Float needs `PVecF64`). Captures the S4 param-sourced thaw and append-only unification as deferred with their real dependencies. |
