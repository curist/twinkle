...
| [2026-07-29-loop-threaded-field-ownership.md](2026-07-29-loop-threaded-field-ownership.md) | 8H follow-up (revised after a spike): let reference-typed field ownership survive a function return so a value from a field-preserving call (`env = put(...)`, `s = s.insert(...)`) proves its field path at the next call. Real gap is variant-aware return-field recovery + `result_ok`, not loops/back-edge (direct loop already works). Fixes sequential/loop/`Set` chains. |
