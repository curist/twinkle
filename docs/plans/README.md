...
| [channels.md](channels.md) | CSP-style `Channel<T>` (unbuffered + bounded, Go-style) on top of the cooperative `Task` scheduler — `send`/`recv`/`close`/`for v in ch`, `Option`-based recv (`T?`), `select` deferred. Replaces the LSP's `Cell`-backed queues + poll loops. Design done |
