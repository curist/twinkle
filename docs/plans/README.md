...
| [2026-07-28-8g-variant-specialization.md](2026-07-28-8g-variant-specialization.md) | Execution plan for sound-uniqueness codegen **Phase 8G**: physically clone a function's ANF body under a proven owned precondition, route callers with a live owned argument to the clone, and let 8A–8F in-place emission fire inside the clone. Physical-clone design confirmed by a spike (vs. virtual); anchors empirically grounded in `compute_variants` probing (record fn `setb` + recursive vector fn `visit` publish; prelude/non-recursive-vector wrappers do not). Remove this row + archive on completion. |
