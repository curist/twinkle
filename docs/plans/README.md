...
| Platform build bundles | `twk build --node`/`--web` emit self-contained runnable bundles under `target/<name>/` (npm + Vite, `main.mjs`); scaffold reverts to Twinkle-only; multi-entry designed/deferred | Planned | [platform-build-bundles.md](platform-build-bundles.md) |
