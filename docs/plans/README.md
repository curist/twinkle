...
| [awfy-codegen-gaps.md](awfy-codegen-gaps.md) | Runtime codegen gaps surfaced by the AWFY suite (`examples/awfy/`) vs Node/Go. Pinpointed from generated WAT: dead `Void`-value `ref.i31` materialization after every statement, un-coalesced ANF local churn, un-inlined `rt_arr__get`, per-field `struct.get` with no loop caching. Ordered attack vectors; baseline metrics recorded 2026-07-02. Complements `vector-perf/` (representation) and `boot-compiler-perf.md` (compile time) |
| [awfy-c5-inplace-vector.md](awfy-c5-inplace-vector.md) | C5 scoping for in-place vector `set_at` (the sieve/bounce lever). Root-caused why in-place doesn't fire for those patterns (non-inlined `set_at` wrapper; interleaved element reads). **Phase 1a + 1b LANDED**: 1a fixed a pre-existing soundness bug where in-place fired on vectors aliased via `slice`/`concat`/record-field/variant/**closure capture** (`refreshed` blanket-overrode taint) and corrupted the alias. 1b made method-form `set_at` in-place-eligible: detect the thin wrapper by shape → register as a COW op → existing loop rewrite swaps the call to `set_in_place` at unique sites (no inliner), plus allow non-escaping `base[i]` reads in the loop. **sieve 35.5→4.9 ms (~7×), bounce 1927→208 ms (~9×).** Phase 2 measured + PAUSED: record-alloc elimination is a non-lever (V8 already scalar-replaces non-escaping GC structs; Ball-reuse ~0%); inliner+SROA is a real but modest ~15% (deferred); the remaining ~2.3× is representation (Lever B / buffer) |
