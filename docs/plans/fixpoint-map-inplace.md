...
- [ ] **Step 1 — Trace one map to its poisoning point.** Pick `exits` in `run_fixpoint`. Dump the function's ownership facts/verdicts and find the first construct that flips `exits` from unique to aliased. Candidates in order of appearance: the `case warm_state { .Some(w) => w.exits … }` init, the `nested_get(exits, …)` reads, the `join_entry_ownership_assumed(blk, exits, …)` call, the `exits[blk.id.id] = …` write, and the return `FixResult.{ exits … } / FixState.{ exits … }`. Record which one the analysis blames.

- [ ] **Step 2 — Read the summaries of the threaded helpers.** For each of `join_entry_ownership`, `join_entry_ownership_assumed`, `join_entry_valid`, `join_entry_prov`, `join_entry_field_own`, `join_entry_path_prov`, `nested_get`, `merge_targeted`, `forward_block`, `same_map`, `seed_param_own`, `seed_param_prov`: dump its summary (param disposition: consumed/borrowed/published; return: fresh/alias(pN)). Tabulate which ones publish or alias a map param. Cross-check `join_entry_ownership_assumed`'s `absent` decision — why no decision was produced at all.

- [ ] **Step 3 — Kill or confirm Hypothesis 2 (cheap).** Temporarily restructure `run_fixpoint`'s return so the five exit maps are **not** shared between `FixResult` and `FixState` (e.g. return only `FixState` and project `FixResult` at the call sites, or vice-versa — do not deep-copy). Re-run `--census --sites` for `run_fixpoint`. If any of the 5 exit maps flips to `reuse(unique)`, the double-embedding is a real (partial) cause and a cheap standalone win; if nothing changes, discard and move on. **Revert the probe regardless** — landing it is a separate decision.

- [ ] **Step 4 — Quantify the ceiling.** Estimate the upside before investing in the fix. Two options: (a) measure a micro-benchmark of `dict$set` vs `dict$set_in_place` at the fixpoint's typical map sizes; or (b) as a *perf-ceiling probe only*, force the fixpoint's dict-sets to `set_in_place` behind a temporary flag (knowing it may be unsound — do **not** trust output; run only long enough to read `summary:roots run`), then A/B `run` against the persistent baseline. This says whether the whole effort is worth, say, 1 s or 4 s off the ~6.8 s. Revert.

- [ ] **Step 5 — Write up + go/no-go.** Append findings to this doc: the pinned cause (which summary/construct), the ceiling number, and a recommendation — one of:
  - **(A) Targeted summary precision:** teach the analysis that the named read-mostly helpers *borrow* their map param and (for the `join_entry_*` family) return a *fresh unique* map. Scope: how many helpers, whether it's expressible in the current summary lattice or needs the sound-uniqueness borrow/return-path work (Case W path liveness).
  - **(B) Source restructure:** if the maps are aliased for a structural reason that a small source change fixes (e.g. the return double-embedding, or splitting a helper so the borrow is intraprocedural like `join_entry_ownership`), do that instead — cheaper, no analysis change.
  - **(C) Not worth it:** ceiling too low or fix too deep; record and stop.
