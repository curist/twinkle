...
| Item | Home |
|---|---|
| Whole-value `MayAliasParams(k)` recovery (the `add_type` caller). **Obligation:** selecting the owned variant is **not sufficient** — `transfer_summarized_call` publishes `MayAliasParams(k)` args unconditionally (`ownership.tw:1160`). Stage 4 **must** add one of: (a) a whole-return owned-handoff representation (e.g. a `[] = OwnedFromParam(k)` return fact), or (b) selected-variant-specific handling that treats `MayAliasParams(k)` as a **move** when the key licenses it. | **Stage 4** |
