...
Run: `target/twk wat /tmp/mvw.tw --func "f307_f"` (find the mangled id via `--list`). Expected AFTER this phase: result type `(ref null $rt_types__PVecI64)`; calls include `rt_arr__get_i64`, `rt_arr__len_i64`, and `rt_arr__set_i64` (the **functional** typed set — in-place is Phase 3); **no** boxed `rt_arr__get`, **no** boxed `rt_arr__set_in_place`, **no** `$rt_types__PVec` for `xs`. (A `BoxedInt` may still appear for the boxed set-value argument — fact 5 — so do NOT assert its global absence; see Phase 5.) If the return is boxed `$rt_types__PVec` or the set is boxed, promotion did not fire — recheck Steps 2–5 (most likely the `AAssign` arm still escaping, Step 4).
