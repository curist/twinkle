...
* Output goes to `target/<name>/<name>.lib.wasm` (grouped under the entry's
  bundle directory; see [platform-build-bundles.md](archive/platform-build-bundles.md)).
...
> **Superseded by [platform-build-bundles.md](archive/platform-build-bundles.md).** The
...
* ~~**Multiple lib entries** per project~~ — **shipped** in
  [platform-build-bundles.md](archive/platform-build-bundles.md): `[lib] entries = [...]`
...
| `boot/commands/build.tw` | `--lib` flag; lib build path → `target/<name>/<name>.lib.wasm` (grouped, per [platform-build-bundles.md](archive/platform-build-bundles.md)); thread "this is a lib build of entry module M" into the pipeline |
...
| `boot/lib/project/scaffold.tw` + `boot/commands/scaffold.tw` | `[lib]` entry only — the JS/web harness moved to `twk build --node`/`--web` output (see [platform-build-bundles.md](archive/platform-build-bundles.md)) |
